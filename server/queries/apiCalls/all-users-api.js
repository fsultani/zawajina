...
(() => {
  const createUser = () => {
    ...
    const data = {
      ...
      country: randomCity.country === "USA" ? "USA" : randomCity.country,
  ...
  const makeApiCalls = async hasError => {
    try {
      ...
      formData.append("image-0", fs.createReadStream(shuffleImages(imagesArray)[0]));
      formData.append("image-1", fs.createReadStream(shuffleImages(imagesArray)[1]));
      formData.append("image-2", fs.createReadStream(shuffleImages(imagesArray)[2]));
      formData.append("image-3", fs.createReadStream(shuffleImages(imagesArray)[3]));
      formData.append("image-4", fs.createReadStream(shuffleImages(imagesArray)[4]));
      formData.append("image-5", fs.createReadStream(shuffleImages(imagesArray)[5]));
